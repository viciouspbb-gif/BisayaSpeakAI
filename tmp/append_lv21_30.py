...
serial = max((int(item["id"]) for item in records if item["id"].isdigit()), default=0)

for level, sentences in lessons:
    for native, translation in sentences:
        serial += 1
        entry = {
            "id": f"{serial:03d}",
            "level": level,
...
print(f"Appended {len(lessons) * 10} records up to id {serial:03d}")
